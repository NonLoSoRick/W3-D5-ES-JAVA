...
const wordsArray = splitMe(frase);
...
const originalString = "Frase esempio incredibile";

const result1 = deleteOne(originalString, true);
const result2 = deleteOne(originalString, false);
...
const email1 = "mail@valida.com";
const email2 = "mailnonvalida";
const isValid1 = isThisAnEmail(email1);
const isValid2 = isThisAnEmail(email2);
...
const rollResult = rollTheDices(7);
...
console.log(`Sono trascorsi ${daysPassed} giorni dalla data ${inputDate}`);
...
function isTodayMyBirthday() {
  const birthday = "10-01";
  const currentDate = new Date();
  const currentDay = currentDate.getDate();
  const currentMonth = currentDate.getMonth() + 1;
  const formattedCurrentDate = `${currentMonth < 10 ? '0' : ''}${currentMonth}-${currentDay < 10 ? '0' : ''}${currentDay}`;
  
  return formattedCurrentDate === birthday;
}
...
function searchByTitle(movies, searchString) {
  const result = movies.filter(movie => movie.Title.toLowerCase().includes(searchString.toLowerCase()));
  return result;
}
